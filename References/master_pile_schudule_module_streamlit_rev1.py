...
def masterpileschudule(s,g,s_output,g_output,project_name,m_n,Phase_extract,boundary):
    ...
    for proj,modelname in zip(project,model_name):
        ...
        # g.calculate()
        # g.save()
        phase_open=getattr(g,Phase_extract[0])
        g.view(phase_open)
        st.write("PLAXIS OUTPUT is now opened")
        g.gotostructures()
    ...
    #    Phase_extract=[g_output.Phase_10]        
        ...
        st.write(dic_index_Phase)
        st.write("Moving now to Extract& Post processing the pile results in the PLAXIS model")
        latest_iteration = st.empty()
        bar = st.progress(0)
        g.gotostructures()
        size_phase=len(Phase_extract)+1
        
        index=0
        for pha in Phase_extract:
            phase=getattr(g_output,pha)
            index=index+1
            percentage=(index/size_phase)

            latest_iteration.text("Extracting results at {}".format(phase.Name.value))
            ...
            for a in g.EmbeddedBeams:
                name_pile=a.Parent.Name.value
                latest_iteration.text("Post-Processing {} data results at {}.".format(name_pile,phase.Name.value))
    ...
    ##############################################################################    
        ...
        latest_iteration.text("Exporting Pile head's results to Excel ")
        ...
        # size_phase=len(Phase_extract)
        # percentage=len(Phase_extract)/size_phase
        latest_iteration.text("Exporting Pile's Post-processed data to Excel... Almost there!")
        bar.progress(0.98)
